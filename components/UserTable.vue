...
const updateUser = async () => {
  try {
    const response = await useFetch(`http://localhost:6969/users/${formData.value.userId}`, {
      method: 'PUT',
      headers: {
...
<template>
        ...
        >
          ...
          <td class="box-border flex justify-center gap-1 py-4">
            ...
            >
              <Icon
                class="text-lg"
                name="ic:twotone-delete-forever"
              />
            </button>
          </td>
        </tr>
    ...
    >
      ...
      <div class="relative z-50 w-full h-full max-w-2xl p-4 md:h-auto">
        ...
        >
          ...
          >
            <h3
              class="text-lg font-semibold text-gray-900 xl:text-2xl xl:font-bold dark:text-white"
            >
              Ubah User
            </h3>
            <button
  ...
  >
    <div class="bg-black opacity-50 fixed top-0 left-0 right-0 z-50 items-center justify-center w-full overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full"></div>
